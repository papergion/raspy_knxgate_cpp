...
void bufferPicLoad(char * decBuffer, char fileWrite)  // fileWrite 1=write config file
{
  ...
  if (busid[0] != 0)
  {
	...
	if (device.Val)
	{
	  ...
	  devtype = axTOchar(stype);
	  busdevType[device.Val] = devtype;
	  if (devtype == 8)
		  busdevType[(int)device.Val+1] = 18;
	  if (devtype == 9)
		  busdevType[(int)device.Val+1] = 19;
	  if (devtype == 4)
		  busdevType[(int)device.Val+1] = 24;
...
//				  }
	  ...
	  if ((devtype == 9) || (devtype == 19) || (devtype == 4) || (devtype == 24)) 			// w6 - aggiorna tapparelle pct su pic
	  {
		...
		requestBuffer[requestLen++] = device.byte.HB;     // device id - line
...
int main(int argc, char *argv[])
{
	...
	char cb;
	strcpy(filename,"knxconfig");
	for (int i=0; i<MAXDEVICE; i++) {busdevType[i] = 0;}
		...
		while (fgets(line, 128, fConfig))
		{
		  ...
		  if (busid[0] != 0)
		  {
			...
			if (device.Val)
			{
			  ...
			  busdevType[device.Val] = axTOchar(stype); 
			  c++;
			}
		  }
		  if (httpcallback[0] == 0)
...
// ----------------------------------------------------------------------------------------------------
	while (1)
	{
		...
		c = read(fduart, &cb, 1);			// lettura da KNXgate
		if (c > 0)
		{
			rx_len = 0;
			rx_buffer[rx_len++] = cb;
...
//		        firstTime = 0;
			...
			if ((rx_buffer[0] == 0xF5) && (rx_buffer[1] == 'y') && (rx_len == 6))   // solo se stringa=   [0xF5] [y] 32 00 12 01
			{
				if (((last_response == 'y') || (last_response == 'a')) && (httpcallback[0] != 0))
				{
				  ...
				  // intero  [8] B4 10 0C 0B 31 E1 00 81 0D
				  // ridotto [0xF5] [y] 0C 0B 31 81
				  // ----------------1---2--3--4--5--
					sprintf(hBuffer, "&from=%02X&to=%02X%02X&cmd=%02X", rx_buffer[2], rx_buffer[3], rx_buffer[4], rx_buffer[5]);
...
KNXserver: got connection from 192.168.2.126 on port: 54858
192.168.2.126 send request: 

GET /json.htm?type=command&param=udevices&script=KNXgate_json.lua&type=12&from=11&to=B8&cmd=00 HTTP/1.1
...
//					memorizzare client_address.sin_addr; <------------------------ 
		...
		// comando §y<source><destaddress><command>
				requestBuffer[requestLen++] = 0x00;				// from device
				requestBuffer[requestLen++] = device.byte.HB;	// to   device - line
				requestBuffer[requestLen++] = device.byte.LB;	// to   device - address
				requestBuffer[requestLen++] = axTOchar(scmd);	// command
		...
		// ------------------------------------------------------------------------------------------------------
			{
			  ...
			  {
				char line[128] = {0};
				deviceX = axTOchar(sdevnum);
				if (verbose) printf("-> devnr %d\n",deviceX);
				if (deviceX == 1)
				{
					...
					{
						if (fConfig)
							printf("-> fopen result: OK\n");
						else
							printf("-> fopen result: KO\n");
					}
/*
					if (fConfig)
					{
		...
		// ------------------------------------------------------------------------------------------------------
			{
			  ...
			  if (p1)
			  {
				...
				if (p6)
					resp = *(p6+5);
				if (verbose) printf("http get, from=%02x,to=%04x,cmd=%02x,resp=%c \n",from,device.Val,cmd,resp);
				last_response = resp;
...
// ------------------------------------------------------------------------------------------------------
			else
			{
			  if (tcpuart == 1)
			  {
				c = write(fduart,tcpBuffer,tcpread);			// scrittura su KNXgate
		        firstTime = 0;
		      }
			}					
// ---------------------------------------------------------------------------------------------------------------------------
