/*
 *  KNXFIRMWARE - download nuovo firmware su pic - input file .bin
 */
#define PROGNAME "KNX_FIRMWARE "
#define VERSION  "1.20"
...
int setFirst(void)
{
  ...
  rx_len = 0;
  rxBufferLoad(100);
  if ((memcmp(rx_buffer,"KNX ",3) == 0) || (force))
  {
	  printf("===============> %s <================\n",rx_buffer);
  }
  else
...
void MsgPrepareFirmware(char len, char * buffer, char log)
{
	write(fduart,buffer,(int)len);			// scrittura su KNXgate
	if (log)
	{
